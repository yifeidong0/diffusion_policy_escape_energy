    ...
    for id in range(num_variations):
        ...
        while j < num_envs:
            ...
            try:
                ...
                centers, rads, start_pos, goal_pos = generate_u_obstacles(fix_obstacles=1)
        ...
        # Save the dataset
        dataset = np.concatenate((np.array(paths), np.array(velocities)), axis=-1)
        trajs_free = torch.tensor(dataset, dtype=torch.float32)
        ...
        print(f"trajs_free.dtype: {trajs_free.dtype}")  # Should output: torch.float64
